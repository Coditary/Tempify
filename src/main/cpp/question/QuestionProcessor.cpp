...
bool review_answers(const TemplateManifest& manifest,
                    const std::map<std::string, std::string>& values,
                    const LuaEngine& lua_engine,
...
void apply_resolved_value(const QuestionDefinition& question,
...
std::map<std::string, std::string> QuestionProcessor::collect(const TemplateManifest& manifest,
    ...
    const auto explicit_imported = question_internal::normalize_assignments(imported_values, manifest.questions, "answer file");
    ...
    for (const auto& question : manifest.questions) {
        ...
        if (const auto imported_value = explicit_imported.find(question.key); imported_value != explicit_imported.end()) {
            apply_resolved_value(question,
                                 imported_value->second,
                                 values,
                                 manifest.questions,
                                 lua_engine_,
                                 "Answer file value");
    ...
    while (true) {
        ...
        if (non_interactive) {
            for (const auto& question : manifest.questions) {
                ...
                const auto default_value = question_internal::default_value_for(question,
                                                                               env_values,
                                                                               explicit_config,
                                                                               values,
                                                                               lua_engine_);
                ...
                throw TempifyError("Missing required answer for '" + question.key + "' while `--non-interactive` is enabled.");
            }
            ++group_index;
            continue;
        }

        if (!question_internal::group_has_interactive_questions(manifest,
                                                                current_group,
                                                                explicit_cli,
                                                                explicit_imported,
                                                                values,
        ...
        for (const auto& question : manifest.questions) {
            ...
            if (explicit_imported.contains(question.key)) {
                if (!values.contains(question.key)) {
                    apply_resolved_value(question,
                                         explicit_imported.at(question.key),
                                         values,
                                         manifest.questions,
                                         lua_engine_,
                                         "Answer file value");
            ...
            const auto default_value = question_internal::default_value_for(question,
                                                                            ...
                                                                            values,
                                                                            lua_engine_);
            const auto answer = question_internal::ask_question(
                question,
                default_value,
                values,
                manifest.questions,
        ...
        if (go_back) {
            ...
            question_internal::clear_group_values(values, manifest, explicit_cli, explicit_imported, previous_group);
        ...
        question_internal::clear_group_values(values,
                                              manifest,
                                              explicit_cli,
                                              explicit_imported,
