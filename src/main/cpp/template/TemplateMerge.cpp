...
bool path_matches_drop_rule(const std::string &candidate, const std::string &drop_path) {
    ...
    return candidate.size() > drop_path.size() && candidate.starts_with(drop_path) &&
...
void merge_scripts(TemplateManifest &result, const TemplateManifest &overlay) {
    for (const auto &script : overlay.scripts) {
        const auto it = std::find_if(result.scripts.begin(), result.scripts.end(),
                                     [&](const ScriptCatalogEntry &existing) { return existing.name == script.name; });
...
void merge_questions(TemplateManifest &result, const TemplateManifest &overlay, const TemplateMergeConfig &policy) {
    ...
    for (const auto &question : overlay.questions) {
        const auto it = std::find_if(result.questions.begin(), result.questions.end(),
                                     [&](const QuestionDefinition &existing) { return existing.key == question.key; });
        ...
        case ConflictStrategy::Error:
            throw TempifyError("Question conflict for key '" + question.key + "' between templates '" +
                               it->source_path.parent_path().parent_path().filename().string() + "' and '" +
                               overlay.info.id + "'");
        }
    }
}
...
void merge_directories(TemplateManifest &result, const TemplateManifest &overlay) {
    for (const auto &directory : overlay.directories) {
        const auto it = std::find_if(
            ...
            [&](const TemplateDirectoryEntry &existing) { return existing.relative_path == directory.relative_path; });
...
void merge_files(TemplateManifest &result, const TemplateManifest &overlay, const TemplateMergeConfig &policy) {
...
void merge_hook(std::optional<std::filesystem::path> &destination, const std::optional<std::filesystem::path> &overlay,
                const std::optional<ConflictStrategy> &policy, const bool overlay_is_local,
                const std::string &error_prefix, const std::string &overlay_id) {
...
TemplateManifest TemplateLoader::merge(const TemplateManifest &base, const TemplateManifest &overlay,
                                       const TemplateMergeConfig &policy, const bool overlay_is_local) const {
    ...
    result.output_path_template =
        !overlay.output_path_template.empty() ? overlay.output_path_template : base.output_path_template;
    ...
    merge_hook(result.pre_hook_path, overlay.pre_hook_path, policy.pre_hook_conflict, overlay_is_local,
               "Pre-hook conflict", overlay.info.id);
    merge_hook(result.post_hook_path, overlay.post_hook_path, policy.post_hook_conflict, overlay_is_local,
               "Post-hook conflict", overlay.info.id);
