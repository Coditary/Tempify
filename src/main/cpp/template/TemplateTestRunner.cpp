...
std::string canonicalize_template_test_lockfile_json(std::string text) {
    const auto replace_json_string_field = [](std::string input, const std::string &key,
...
std::vector<FixtureDefinition> discover_fixtures(const TemplateManifest &manifest,
    ...
    for (const auto &root : fixture_roots) {
        ...
        if (!std::filesystem::is_directory(fixture.snapshot_root)) {
            throw TempifyError("Fixture '" + fixture.name +
                               "' missing snapshot directory: " + fixture.snapshot_root.string());
        }
        if (std::filesystem::exists(fixture.answers_path) && !std::filesystem::is_regular_file(fixture.answers_path)) {
            throw TempifyError("Fixture '" + fixture.name +
                               "' answers path is not file: " + fixture.answers_path.string());
        }
        if (std::filesystem::exists(fixture.lockfile_snapshot_path) &&
            !std::filesystem::is_regular_file(fixture.lockfile_snapshot_path)) {
            throw TempifyError("Fixture '" + fixture.name +
                               "' lock snapshot path is not file: " + fixture.lockfile_snapshot_path.string());
        }
...
std::string describe_content_mismatch(const std::string &relative_path, const std::string &expected,
    ...
    for (std::size_t index = 0; index < line_count; ++index) {
        ...
        if (expected_line != actual_line) {
            std::ostringstream stream;
            stream << "content mismatch in " << relative_path << " at line " << (index + 1) << ": expected `"
                   << preview_line(expected_line) << "`, got `" << preview_line(actual_line) << '`';
            return stream.str();
        }
    }
...
void compare_expected_file_set(const std::vector<std::string> &expected_files,
                               const std::vector<std::string> &actual_files, const FixtureDefinition &fixture) {
    std::vector<std::string> missing_files;
    std::vector<std::string> unexpected_files;
    std::set_difference(expected_files.begin(), expected_files.end(), actual_files.begin(), actual_files.end(),
                        std::back_inserter(missing_files));
    std::set_difference(actual_files.begin(), actual_files.end(), expected_files.begin(), expected_files.end(),
...
bool compare_lockfile_snapshot(const FixtureDefinition &fixture, const TemplateManifest &manifest,
                               const BuildPlan &plan, const std::map<std::string, std::string> &values) {
    ...
    const std::string actual = canonicalize_template_test_lockfile_json(
        format_generation_lock_json(manifest, plan, values, HookAcceptance::Yes, false));
    if (expected != actual) {
        throw TempifyError("Fixture '" + fixture.name +
                           "' lockfile mismatch: " + describe_content_mismatch("lock.json", expected, actual));
    }
    return true;
}

void write_fixture_snapshot(const FixtureDefinition &fixture, const TemplateManifest &manifest, const BuildPlan &plan,
    ...
    if (std::filesystem::is_regular_file(fixture.lockfile_snapshot_path)) {
        write_file(fixture.lockfile_snapshot_path, canonicalize_template_test_lockfile_json(format_generation_lock_json(
                                                       manifest, plan, values, HookAcceptance::Yes, false)));
    }
}

SnapshotComparisonResult compare_snapshot_tree(const FixtureDefinition &fixture, const TemplateManifest &manifest,
    ...
    for (const auto &relative_path : expected_files) {
        ...
        if (expected != actual) {
            throw TempifyError("Fixture '" + fixture.name +
                               "' snapshot mismatch: " + describe_content_mismatch(relative_path, expected, actual));
        }
    }
...
TemplateTestRunner::TemplateTestRunner(const LuaEngine &lua_engine, const PrebyteRenderer &renderer)
    : lua_engine_(lua_engine), renderer_(renderer) {}

std::vector<TemplateFixtureListing>
TemplateTestRunner::list_fixtures(const TemplateManifest &manifest,
...
TemplateTestReport TemplateTestRunner::run(const TemplateManifest &manifest,
    ...
    for (const auto &fixture : fixtures) {
        ...
        try {
            const auto values = question_processor.collect(manifest, {}, {}, load_fixture_answers(fixture), true, true);
            ...
            report.total_snapshot_artifact_count +=
                comparison.file_count + (comparison.includes_lockfile_snapshot ? 1U : 0U);
        ...
        result.elapsed_ms = static_cast<std::size_t>(
            std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - fixture_start)
                .count());
...
TemplateTestReport TemplateTestRunner::update_snapshots(const TemplateManifest &manifest,
    ...
    for (const auto &fixture : fixtures) {
        ...
        try {
            ...
            report.total_snapshot_artifact_count +=
                result.snapshot_file_count + (result.includes_lockfile_snapshot ? 1U : 0U);
        } catch (const TempifyError &error) {
            const FixtureFailure failure = classify_failure(error.what());
            result.failure_code = failure.code;
            result.failure_kind = failure.kind;
...
std::string format_template_test_report(const TemplateTestReport &report) {
    ...
    if (!report.fixtures.empty()) {
        stream << " (" << report.total_snapshot_artifact_count << " snapshot artifacts, " << report.elapsed_ms
               << " ms)";
    }
    stream << '\n';
    return stream.str();
