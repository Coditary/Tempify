...
TEST_CASE(TempifyApp_diff_json_outputs_machine_readable_report) {
    ...
    REQUIRE_EQ(app.run({
                   ...
                   target.path().string(),
                   "--set",
                   "project_name=Diff Json App",
                   "--set",
                   "name_slug=diff-json-app",
                   "--set",
                   "namespace=diff_json_ns",
                   "--set",
                   "include_ci=false",
                   "--set",
                   "author=Diff Json Tester",
               }),
               0);
...
TEST_CASE(TempifyApp_diff_classifies_template_updates_from_generation_baseline) {
    ...
    ScopedTempifyDataHome data_home(std::filesystem::temp_directory_path() /
                                    "tempify-app-diff-template-update-data-home");
    ...
    REQUIRE_EQ(app.run({
                   ...
                   target.path().string(),
                   "--set",
                   "project_name=Template Update App",
                   "--set",
                   "name_slug=template-update-app",
                   "--set",
                   "namespace=template_update_ns",
                   "--set",
                   "include_ci=false",
                   "--set",
                   "author=Old Author",
...
TEST_CASE(TempifyApp_diff_detects_delete_candidates_from_lockfile) {
    ...
    REQUIRE_EQ(app.run({
                   ...
                   "--set",
                   "name_slug=diff-delete-app",
                   "--set",
                   "namespace=diff_delete_ns",
                   "--set",
                   "include_ci=false",
                   "--set",
                   "author=Diff Delete Tester",
               }),
               0);
...
TEST_CASE(TempifyApp_diff_surfaces_origin_template_mismatch_for_reapply) {
    ...
    ScopedTempifyDataHome data_home(std::filesystem::temp_directory_path() /
                                    "tempify-app-diff-origin-mismatch-data-home");
    ...
    REQUIRE_EQ(app.run({
                   ...
                   target.path().string(),
                   "--set",
                   "project_name=Origin Mismatch App",
                   "--set",
                   "project_slug=origin-mismatch-app",
                   "--set",
    ...
    REQUIRE_EQ(app.run({
                   ...
                   "project_name=Origin Mismatch App",
                   "--set",
                   "name_slug=origin-mismatch-app",
                   "--set",
                   "namespace=origin_mismatch_ns",
                   "--set",
                   "include_ci=false",
                   "--set",
...
TEST_CASE(TempifyApp_diff_surfaces_pre_1_0_minor_upgrade_policy_for_update_workflow) {
    ...
    ScopedTempifyDataHome data_home(std::filesystem::temp_directory_path() /
                                    "tempify-app-diff-version-change-data-home");
    ...
    REQUIRE_EQ(app.run({
                   ...
                   target.path().string(),
                   "--set",
                   "project_name=Version Change App",
                   "--set",
                   "name_slug=version-change-app",
                   "--set",
                   "namespace=version_change_ns",
                   "--set",
                   "include_ci=false",
                   "--set",
                   "author=Version Tester",
               }),
               0);
...
TEST_CASE(TempifyApp_diff_treats_semver_prerelease_release_transition_as_upgrade) {
    ...
    ScopedTempifyDataHome data_home(std::filesystem::temp_directory_path() /
                                    "tempify-app-diff-semver-prerelease-data-home");
    tempify::TempifyApp app;
    REQUIRE_EQ(app.run({
                   "basic_cpp",
                   target.path().string(),
                   "--set",
                   "project_name=Semver Prerelease App",
                   "--set",
                   "name_slug=semver-prerelease-app",
                   "--set",
                   "namespace=semver_prerelease_ns",
                   "--set",
                   "include_ci=false",
                   "--set",
                   "author=Semver Tester",
               }),
               0);
...
TEST_CASE(TempifyApp_diff_treats_semver_build_metadata_change_as_equivalent_version) {
    ...
    REQUIRE_EQ(app.run({
                   ...
                   target.path().string(),
                   "--set",
                   "project_name=Semver Build App",
                   "--set",
                   "name_slug=semver-build-app",
                   "--set",
                   "namespace=semver_build_ns",
                   "--set",
                   "include_ci=false",
                   "--set",
                   "author=Semver Tester",
...
TEST_CASE(TempifyApp_diff_surfaces_major_upgrade_policy_review) {
    ...
    const std::filesystem::path template_path =
        create_required_only_template_with_version(template_root.path(), "1.0.0");
    ...
    REQUIRE_EQ(app.run({
                   ...
                   target.path().string(),
                   "--set",
                   "required_name=Major Upgrade App",
               }),
               0);
