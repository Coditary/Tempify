...
TEST_CASE(LuaEngine_load_template_info_and_ignores_pbc_artifacts) {
    tempify::LuaEngine lua_engine;
    const tempify::TemplateInfo info =
        lua_engine.load_template_info(tempify::test_support::test_template_path("basic_cpp"));
    const tempify::TemplateManifest manifest =
        lua_engine.load_partial_manifest(tempify::test_support::test_template_path("basic_cpp"));
...
TEST_CASE(LuaEngine_run_hook_supports_file_ops_process_string_process_file_and_script_catalog) {
    ...
    REQUIRE(
        read_text_file(build_root.path() / "nested" / "copied.txt").find("This should stay raw: {{ project_name }}") !=
        std::string::npos);
...
TEST_CASE(LuaEngine_load_partial_manifest_throws_for_invalid_layout_and_questions_files) {
    ...
    ScopedDirectoryCleanup question_workspace(std::filesystem::temp_directory_path() /
                                              "tempify-lua-invalid-questions-test");
    write_text_file(question_workspace.path() / "template.lua",
                    "return { id = 'question_bad', source_dir = 'files' }\n");
    std::filesystem::create_directories(question_workspace.path() / "files");
    write_text_file(question_workspace.path() / "questions.lua",
                    "return { order = { 'Project' }, groups = { Project = 'bad' } }\n");
