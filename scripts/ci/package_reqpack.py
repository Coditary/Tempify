...
def build_metadata(version: str, platform: str, arch: str, archive_name: str, size_compressed: int, size_installed: int) -> dict[str, object]:
    ...
    return {
        ...
        "release": 1,
        "revision": 0,
        "summary": "Tempify template engine CLI",
        "description": "Local-first template engine CLI with bundled ReqPack Tempify plugin.",
        "license": "MIT",
        "architecture": arch,
        "system": [SYSTEM_BY_PLATFORM[platform]],
        "vendor": VENDOR,
        "maintainerEmail": MAINTAINER_EMAIL,
        "url": f"https://github.com/Coditary/Tempify/releases/download/{release_tag}/{archive_name}",
        "homepage": "https://github.com/Coditary/Tempify",
        "sourceUrl": "https://github.com/Coditary/Tempify",
        "buildDate": build_date,
        "tags": ["cli", "templating", "tempify", "reqpack"],
        "binaries": ["tempify"],
        "payload": {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Package Tempify as ReqPack .rqp archive")
    ...
    with tempfile.TemporaryDirectory(prefix=f"tempify-rqp-{args.platform}-{args.arch}-", dir=output_dir) as temp_dir:
        ...
        payload_doc_dir = payload_root / "share" / "doc" / "tempify"
        payload_plugin_dir = payload_root / "share" / "reqpack" / "plugins" / "tempify"
        ...
        binary_target = payload_bin_dir / "tempify"
